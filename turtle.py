...
from community.behaviours.communication import Receiver
...
if __name__ == "__main__":
    ...
    st.createMultiChoiceTransition()\
        .fromState(LISTEN_ORDER)\
            .ifCondition(EVENT_ORDER_SIT_DOWN).goTo(SIT_DOWN) \
            .elifCondition(EVENT_ORDER_STAND_UP).goTo(STAND_UP)\
            .elifCondition(EVENT_ORDER_SAY_HI).goTo(SAY_HI)\
        .create()

    st.createSimpleTransition(SIT_DOWN, LISTEN_CALL, 0)
    st.createSimpleTransition(STAND_UP, LISTEN_CALL, 1)
    st.createSimpleTransition(SAY_HI, LISTEN_CALL, 2)
    ...
    naoAgent = AgentBuilder()\
        ...
        .create()
    # implementer le communicator dans community.behaviours.communication

    naoAgent.raiseEvent(EVENT_ORDER_LISTEN)
    naoAgent.setData("dataKey", value)
    naoAgent.getData("dataKey")

    # l'agent est dans community.core

    naoAgent.behaviours["BehaviourName"] # récupérer un behaviour
